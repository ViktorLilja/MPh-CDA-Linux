...
SPECIFIC FOR THIS FORK OF MPH:
First searches through custom installation locations for Chalmers CDA computers
and NAISS compute clusters.
...
def find_backends_cda_linux():
    """Searches for Comsol installations within Linux CDA system."""

    # Collect all information in a list.
    print("Searching for COMSOL in Linux CDA")
    ...
    process = subprocess.run([comsol, 'server', '--version'], stdout=subprocess.PIPE)
    if process.returncode != 0:
        log.warning('Querying version information failed.')
    ...
    try:
        ...
        log.warning("Unable to parse version string, setting arbitrary values")
...
def find_backends_pdc_linux():
    """Searches for Comsol installations on the PDC system."""

    # Collect all information in a list.
    print("Searching for COMSOL within PDC file system")
    
    # Manually specify paths to installation within filesystem
    base   = "/cfs/klemming/projects/snic/nanoinversedesign/sw/maxwell/62"
    root   = Path(base + "/")
    comsol = Path(base + "/bin/comsol")
    java   = Path(base + "/java/glnxa64/jre/bin")
    jvm    = Path(base + "/java/glnxa64/jre/lib/server/libjvm.so")
    api    = Path(base + "/plugins")
    lib    = Path(base + "/glnxa64")
    gra    = Path(base + "/ext/graphicsmagick/glnxa64")
    ...
    try:
        ...
        log.warning("Unable to parse version string, setting arbitrary values")
        (name, major, minor, patch, build) = ("6.1",6,1,None,666)
...
def find_backends():
    ...
    # First look in custom search path
    root_name = os.path.abspath(os.curdir).split("/")[1]
    if root_name == "chalmers" and system == "Linux": # Chalmers CDA Linux
        return find_backends_cda_linux()
    if root_name == "cfs" and system == "Linux": # PDC
        return find_backends_pdc_linux()
